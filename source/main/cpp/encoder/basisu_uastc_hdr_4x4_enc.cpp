...
#include "ctff/private/encoder/basisu_uastc_hdr_4x4_enc.h"
...
namespace basisu
{
    ...
    static void pack_mode11(const vec4F* pBlock_linear_colors, const half_float pBlock_pixels_half[16][3], const vec4F pBlock_pixels_q16[16], basisu::vector<astc_hdr_4x4_pack_results>& all_results, const uastc_hdr_4x4_codec_options& coptions,
                            uint32_t first_weight_ise_range, uint32_t last_weight_ise_range, bool constrain_ise_weight_selectors)
    {
        ...
        for (uint32_t weight_ise_range = first_weight_ise_range; weight_ise_range <= last_weight_ise_range; weight_ise_range++)
        {
            ...
            double trial_error = encode_astc_hdr_block_mode_11(16, pBlock_pixels_half, pBlock_pixels_q16, weight_ise_range, trial_submode11, BIG_FLOAT_VAL, trial_endpoints, trial_weights, coptions, direct_only, endpoint_ise_range,
                                                               coptions.m_mode11_uber_mode && (weight_ise_range >= astc_helpers::BISE_4_LEVELS) && coptions.m_allow_uber_mode, constrain_ise_weight_selectors, coptions.m_first_mode11_submode,
                                                               coptions.m_last_mode11_submode, false, cOrdinaryLeastSquares);
    ...
    static void pack_mode7_single_part(const half_float pBlock_pixels_half[16][3], const vec4F pBlock_pixels_q16[16], basisu::vector<astc_hdr_4x4_pack_results>& all_results, const uastc_hdr_4x4_codec_options& coptions,
    ...
    static bool estimate_partition(const half_float pBlock_pixels_half[16][3], int* pBest_parts, uint32_t num_best_parts)
    {
    ...
    static void pack_mode7_2part(const half_float pBlock_pixels_half[16][3], const vec4F pBlock_pixels_q16[16], basisu::vector<astc_hdr_4x4_pack_results>& all_results, const uastc_hdr_4x4_codec_options& coptions, int num_estimated_partitions,
                                 const int* pEstimated_partitions, uint32_t first_weight_ise_range, uint32_t last_weight_ise_range)
    {
        ...
        for (uint32_t part_index_iter = first_part_index; part_index_iter < last_part_index; ++part_index_iter)
        {
            ...
            for (uint32_t weight_ise_range = first_weight_ise_range; weight_ise_range <= last_weight_ise_range; weight_ise_range++)
            {
                ...
                {
                    total_trial_err += encode_astc_hdr_block_mode_7(num_part_pixels[pack_part_index], part_pixels_half[pack_part_index], part_pixels_q16[pack_part_index], weight_ise_range, trial_submode7[pack_part_index], BIG_FLOAT_VAL,
    ...
    static void pack_mode11_2part(const half_float pBlock_pixels_half[16][3], const vec4F pBlock_pixels_q16[16], basisu::vector<astc_hdr_4x4_pack_results>& all_results, const uastc_hdr_4x4_codec_options& coptions, int num_estimated_partitions,
                                  const int* pEstimated_partitions)
    {
        ...
        for (uint32_t part_index_iter = first_part_index; part_index_iter < last_part_index; ++part_index_iter)
        {
            ...
            for (uint32_t weight_ise_range = coptions.m_first_mode11_part2_weight_ise_range; weight_ise_range <= coptions.m_last_mode11_part2_weight_ise_range; weight_ise_range++)
            {
                ...
                for (uint32_t pack_part_index = 0; pack_part_index < 2; pack_part_index++)
                {
                    total_trial_err += encode_astc_hdr_block_mode_11(num_part_pixels[pack_part_index], part_pixels_half[pack_part_index], part_pixels_q16[pack_part_index], weight_ise_range, trial_submode11[pack_part_index], BIG_FLOAT_VAL,
                                                                     &trial_endpoints[pack_part_index][0], &trial_weights[pack_part_index][0], coptions, direct_only, ise_endpoint_range,
    ...
    bool astc_hdr_4x4_enc_block(const float* pRGBPixels, const basist::half_float* pRGBPixelsHalf, const uastc_hdr_4x4_codec_options& coptions, basisu::vector<astc_hdr_4x4_pack_results>& all_results)
    {
        ...
        if (!is_solid)
        {
            ...
            else
            {
                if (coptions.m_use_mode11_part1)
                {
                    ...
                    if (coptions.m_last_mode11_weight_ise_range >= astc_helpers::BISE_12_LEVELS)
                    {
                        ...
                        pack_mode11(block_linear_colors, *pBlock_pixels_half, block_pixels_q16, all_results, coptions, maximum<uint32_t>(coptions.m_first_mode11_weight_ise_range, astc_helpers::BISE_12_LEVELS), coptions.m_last_mode11_weight_ise_range,
                                    true);
                    }
            ...
            if (coptions.m_use_mode7_part2)
            {
                ...
                pack_mode7_2part(*pBlock_pixels_half, block_pixels_q16, all_results, coptions, have_est ? coptions.m_max_estimated_partitions : 0, best_parts, coptions.m_first_mode7_part2_weight_ise_range, coptions.m_last_mode7_part2_weight_ise_range);
                ...
                if (all_results.size() == cur_num_results)
                {
                    ...
                    {
                        pack_mode7_2part(*pBlock_pixels_half, block_pixels_q16, all_results, coptions, have_est ? coptions.m_max_estimated_partitions : 0, best_parts, astc_helpers::BISE_4_LEVELS, astc_helpers::BISE_4_LEVELS);
                    }
                }
            }
    ...
    bool astc_hdr_4x4_refine_weights(const half_float* pSource_block, astc_hdr_4x4_pack_results& cur_results, const uastc_hdr_4x4_codec_options& coptions, float bc6h_weight, bool* pImproved_flag)
    {
        if (pImproved_flag)
