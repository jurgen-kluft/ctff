...
#include "ctff/private/encoder/pvpngreader.h"
...
#define PNG_INFLATE_SRC_BUF_SIZE (4096)
    ...
    public:
        ...
        virtual uint64_t get_size() { return m_buf.size(); }

        virtual uint64_t tell() { return m_ofs; }
    ...
    public:
        ...
        virtual uint64_t get_size() { return m_buf_size; }
...
#    define fseek64 fseeko
    ...
    public:
        ...
        virtual bool seek(uint64_t ofs) { return fseek64(m_pFile, ofs, SEEK_SET) == 0; }

        virtual size_t write(const void* pBuf, size_t len) { return (size_t)fwrite(pBuf, 1, len, m_pFile); }

        virtual size_t read(void* pBuf, size_t len) { return (size_t)fread(pBuf, 1, len, m_pFile); }
    };
...
#define TEMP_BUF_SIZE (384)
    ...
    int png_decoder::unchunk_data(uint8_t* buf, uint32_t bytes, uint32_t* ptr_bytes_read)
    {
        ...
        while (bytes_read != bytes)
        {
            if (!m_chunk_flag)
            {
                ...
                if ((m_chunk_name[0] != 'I') || (m_chunk_name[1] != 'D') || (m_chunk_name[2] != 'A') || (m_chunk_name[3] != 'T'))
                {
    ...
    inline void png_decoder::adam7_write_pixel_8(int x, int y, int c) { m_adam7_image_buf[x + y * m_dst_bytes_per_line] = (uint8_t)c; }
    ...
    int png_decoder::decompress_line(uint32_t* bytes_decoded)
    {
        ...
        for (;;)
        {
            ...
            for (;;)
            {
                ...
                if ((m_inflate_src_buf_ofs == m_inflate_src_buf_size) && (m_inflate_eof_flag == FALSE))
                    break;
            }
        }
    }
    ...
    int png_decoder::find_iend_chunk()
    {
        ...
        for (;;)
        {
            if ((m_chunk_name[0] == 'I') && (m_chunk_name[1] == 'E') && (m_chunk_name[2] == 'N') && (m_chunk_name[3] == 'D'))
                break;
    ...
    int png_decoder::png_decode(void** ppImg_ptr, uint32_t* pImg_len)
    {
        ...
        if (m_pass_y_left == 0)
        {
            ...
            for (;;)
            {
                ...
                if (((m_pass_y_left = m_adam7_pass_size_y[m_adam7_pass_num]) != 0) && ((m_pass_x_size = m_adam7_pass_size_x[m_adam7_pass_num]) != 0))
                    break;
            }
            ...
            switch (m_adam7_pass_num)
            {
                ...
                case 1:
                case 3:
                case 5: m_adam7_pass_y = 0; break;
                case 2: m_adam7_pass_y = 4; break;
                case 4: m_adam7_pass_y = 2; break;
                case 6: m_adam7_pass_y = 1; break;
            }
        ...
        switch (m_pCur_line_buf[0])
        {
            ...
            default: return terminate(PNG_UNS_PREDICTOR);
        }
    ...
    void png_decoder::png_decode_end() { uninitialize(); }
    ...
    int png_decoder::read_signature()
    {
        ...
        if ((m_temp_buf[0] != 137) || (m_temp_buf[1] != 80) || (m_temp_buf[2] != 78) || (m_temp_buf[3] != 71) || (m_temp_buf[4] != 13) || (m_temp_buf[5] != 10) || (m_temp_buf[6] != 26) || (m_temp_buf[7] != 10))
        {
            return terminate(PNG_UNKNOWNTYPE);
        }
        ...
        switch (m_ihdr.m_color_type)
        {
            case PNG_COLOR_TYPE_GREYSCALE:
            {
                switch (m_ihdr.m_bit_depth)
                {
                    ...
                    default: return terminate(PNG_BAD_BIT_DEPTH);
                }
            ...
            default: return terminate(PNG_UNS_COLOR_TYPE);
        }
    ...
    int png_decoder::read_plte_chunk()
    {
        ...
        if ((m_ihdr.m_color_type == PNG_COLOR_TYPE_GREYSCALE) || (m_ihdr.m_color_type == PNG_COLOR_TYPE_GREYSCALE_ALPHA))
            return terminate(PNG_BAD_PLTE_CHUNK);
    ...
    png_decoder::png_decoder() { clear(); }

    png_decoder::~png_decoder() { uninitialize(); }
    ...
    static inline uint8_t get_709_luma(uint32_t r, uint32_t g, uint32_t b) { return (uint8_t)((13938U * r + 46869U * g + 4729U * b + 32768U) >> 16U); }
    ...
    bool get_png_info(const void* pImage_buf, size_t buf_size, png_info& info)
    {
        ...
        {
            case PNG_COLOR_TYPE_GREYSCALE: info.m_num_chans = dec.m_trns_flag ? 2 : 1; break;
            case PNG_COLOR_TYPE_GREYSCALE_ALPHA: info.m_num_chans = 2; break;
            case PNG_COLOR_TYPE_PALETTIZED:
            case PNG_COLOR_TYPE_TRUECOLOR: info.m_num_chans = dec.m_trns_flag ? 4 : 3; break;
            case PNG_COLOR_TYPE_TRUECOLOR_ALPHA: info.m_num_chans = 4; break;
    ...
    void* load_png(const void* pImage_buf, size_t buf_size, uint32_t desired_chans, uint32_t& width, uint32_t& height, uint32_t& num_chans)
    {
        ...
        switch (colortype)
        {
            case PNG_COLOR_TYPE_GREYSCALE: num_chans = dec.m_trns_flag ? 2 : 1; break;
            case PNG_COLOR_TYPE_GREYSCALE_ALPHA: num_chans = 2; break;
            case PNG_COLOR_TYPE_PALETTIZED:
            case PNG_COLOR_TYPE_TRUECOLOR: num_chans = dec.m_trns_flag ? 4 : 3; break;
            case PNG_COLOR_TYPE_TRUECOLOR_ALPHA: num_chans = 4; break;
            default: assert(0); break;
        }
                ...
                case PNG_COLOR_TYPE_TRUECOLOR_ALPHA:
                {
                    ...
                    switch (desired_chans)
                    {
                        ...
                        case 3:
                            ...
                            break;
                        case 4: memcpy(pDst, pLine, pitch); break;
                    }
