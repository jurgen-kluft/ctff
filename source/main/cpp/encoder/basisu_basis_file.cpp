...
#include "ctff/private/encoder/basisu_basis_file.h"
...
namespace basisu
{
    ...
    bool basisu_file::init(const basisu_backend_output& encoder_output, basist::basis_texture_type tex_type, uint32_t userdata0, uint32_t userdata1, bool y_flipped, uint32_t us_per_frame)
    {
        ...
        if (!encoder_output.m_uses_global_codebooks)
        {
            check_size = (uint64_t)sizeof(basist::basis_file_header) + (uint64_t)sizeof(basist::basis_slice_desc) * slice_descs.size() + (uint64_t)encoder_output.m_endpoint_palette.size() + (uint64_t)encoder_output.m_selector_palette.size() +
                         (uint64_t)encoder_output.m_slice_image_tables.size();
        }
        else
        {
            check_size = (uint64_t)sizeof(basist::basis_file_header) + (uint64_t)sizeof(basist::basis_slice_desc) * slice_descs.size() + (uint64_t)encoder_output.m_slice_image_tables.size();
        }
