...
#define QUADRATIC_SUPPORT 1.5f
    ...
    static float quadratic_interp_filter(float t) { return quadratic(t, 1.0f); }

    static float quadratic_approx_filter(float t) { return quadratic(t, .5f); }

    static float quadratic_mix_filter(float t) { return quadratic(t, .8f); }
...
#define CATMULL_ROM_SUPPORT (2.0f)
    ...
    static double blackman_exact_window(double x) { return 0.42659071f + 0.49656062f * cos(M_PI * x) + 0.07684867f * cos(2.0f * M_PI * x); }
