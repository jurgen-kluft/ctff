...
#include "ctff/private/encoder/basisu_astc_hdr_6x6_enc.h"
#include "ctff/private/encoder/basisu_enc.h"
#include "ctff/private/encoder/basisu_astc_hdr_common.h"
#include "ctff/private/encoder/basisu_math.h"
#include "ctff/private/encoder/basisu_resampler.h"
#include "ctff/private/encoder/basisu_resampler_filters.h"

#define MINIZ_HEADER_FILE_ONLY
#define MINIZ_NO_ZLIB_COMPATIBLE_NAMES
#include "ctff/private/encoder/basisu_miniz.h"

#include "ctff/private/3rdparty/android_astc_decomp.h"
...
static float inversePQ(float E)
{
    ...
    static inline void linear_rgb_to_itp(const vec3F& rgb_in, vec3F& itp, const astc_hdr_6x6_global_config& cfg) { linear_rgb_to_ictcp(rgb_in, itp, true, cfg.m_rec2020_bt2100_color_gamut); }
...
static void ictcp_to_linear_rgb(const vec3F& ictcp, vec3F& rgb, bool itp_flag = false)
{
    ...
    struct half_vec3
    {
        ...
        inline half_vec3(const half_vec3& other) { *this = other; }
        ...
        inline void clear() { clear_obj(m_vals); }
        ...
        template <typename T> inline half_vec3& set_vec(const T& vec)
        {
        ...
        template <typename T> inline T get_vec() const { return T(basist::half_to_float(m_vals[0]), basist::half_to_float(m_vals[1]), basist::half_to_float(m_vals[2])); }

        inline basist::half_float operator[](uint32_t c) const
        {
            assert(c < 3);
            return m_vals[c];
        }
        inline basist::half_float& operator[](uint32_t c)
        {
            assert(c < 3);
            return m_vals[c];
        }
    ...
    struct half_vec4
    {
        ...
        inline half_vec4(const half_vec4& other) { *this = other; }
        ...
        template <typename T> inline half_vec4& set_vec(const T& vec)
        {
            m_vals[0] = basist::float_to_half(vec[0]);
            m_vals[1] = basist::float_to_half(vec[1]);
            m_vals[2] = basist::float_to_half(vec[2]);
        ...
        template <typename T> inline T get_vec() const { return T(basist::half_to_float(m_vals[0]), basist::half_to_float(m_vals[1]), basist::half_to_float(m_vals[2]), basist::half_to_float(m_vals[3])); }
    ...
    static uint8_t g_part3_mapping[NUM_PART3_MAPPINGS][3] = {{0, 1, 2}, {1, 2, 0}, {2, 0, 1}, {0, 2, 1}, {1, 0, 2}, {2, 1, 0}};

    struct partition_pattern_vec
    {
        uint8_t m_parts[6 * 6];

        partition_pattern_vec() { clear(); }

        partition_pattern_vec(const partition_pattern_vec& other) { *this = other; }

        void clear() { memset(m_parts, 0, sizeof(m_parts)); }
        ...
        float get_distance(const partition_pattern_vec& other) const { return sqrtf((float)get_squared_distance(other)); }
        ...
        bool operator==(const partition_pattern_vec& rhs) const { return memcmp(m_parts, rhs.m_parts, sizeof(m_parts)) == 0; }

        operator size_t() const { return basisu::hash_hsieh(m_parts, sizeof(m_parts)); }
    };
...
#define BRUTE_FORCE_PART_SEARCH (0)
    ...
    public:
        ...
        public:
            ...
            size_t get_size() const { return m_cur_size; }

            bool empty() const { return !m_cur_size; }
            ...
            void clear() { m_cur_size = 0; }

            void reserve(uint32_t n) { BASISU_NOTE_UNUSED(n); }
    ...
    private:
        ...
        void find_nearest_at_node(int node_index, uint32_t num_desired_pats, const partition_pattern_vec* pDesired_pats, result_queue& results, uint32_t max_results)
        {
            ...
            if (best_dist_to_vantage <= m_nodes[node_index].m_dist)
            {
                ...
                {
                    if ((results.get_size() < max_results) || ((m_nodes[node_index].m_dist - best_dist_to_vantage) <= results.get_highest_dist()))
                    {
        ...
        std::pair<int, float> find_best_vantage_point(uint32_t num_unique_pats, const partition_pattern_vec* pUnique_pats, const uint_vec& pat_indices)
        {
            ...
            for (uint32_t pat_indices_iter = 0; pat_indices_iter < n; pat_indices_iter++)
            {
                ...
                if ((split_metric > best_split_metric) || ((split_metric == best_split_metric) && (s.m_var > best_split_var)))
                {
    ...
    struct partition
    {
        ...
        inline bool operator<(const partition& p) const { return m_p < p.m_p; }

        inline bool operator==(const partition& p) const { return m_p == p.m_p; }

        inline operator size_t() const { return hash_hsieh((const uint8_t*)&m_p, sizeof(m_p)); }
    };
    ...
    static bool estimate_partition2_6x6(const basist::half_float pBlock_pixels_half[][3], int* pBest_parts, uint32_t num_best_parts)
    {
    ...
    static bool encode_block_2_subsets(trial_result res[2], uint32_t grid_w, uint32_t grid_h, uint32_t cem, uint32_t weights_ise_range, uint32_t endpoints_ise_range, const half_vec3* pBlock_pixels_half, const vec4F* pBlock_pixels_q16,
                                       astc_hdr_codec_base_options& coptions, bool uber_mode_flag, int unique_pat_index, uint32_t comp_level, opt_mode_t mode11_opt_mode, bool refine_endpoints_flag)
    {
        ...
        for (uint32_t part_iter = 0; part_iter < 2; part_iter++)
        {
            ...
            if (cem == 7)
            {
                e = encode_astc_hdr_block_mode_7(part_total_pixels[part_iter], (basist::half_float(*)[3])part_half_pixels[part_iter], (vec4F*)part_pixels_q16[part_iter], best_log_blk.m_weight_ise_range, best_submode[part_iter], BIG_FLOAT_VAL,
                                                 blk_endpoints[part_iter], blk_weights[part_iter], coptions, best_log_blk.m_endpoint_ise_range);
            }
            else
            {
                assert(cem == 11);

                e = encode_astc_hdr_block_mode_11(part_total_pixels[part_iter], (basist::half_float(*)[3])part_half_pixels[part_iter], (vec4F*)part_pixels_q16[part_iter], best_log_blk.m_weight_ise_range, best_submode[part_iter], BIG_FLOAT_VAL,
                                                  blk_endpoints[part_iter], blk_weights[part_iter], coptions, false, best_log_blk.m_endpoint_ise_range, uber_mode_flag, false, -1, 7, false, mode11_opt_mode);
            }
        ...
        else
        {
            ...
            if ((refine_endpoints_flag) && (comp_level >= MIN_REFINE_LEVEL) && ((grid_w < 6) || (grid_h < 6)))
            {
                ...
                for (uint32_t part_iter = 0; part_iter < 2; part_iter++)
                {
                    bool refine_status = refine_endpoints(cem, endpoints_ise_range,
    ...
    static bool estimate_partition3_6x6(const basist::half_float pBlock_pixels_half[][3], int* pBest_parts, uint32_t num_best_parts)
    {
        ...
        for (uint32_t s = 0; s < NUM_ITERS; s++)
        {
            ...
            for (uint32_t i = 0; i < BLOCK_T; i++)
            {
                float d[NUM_SUBSETS] = {training_vecs[i].squared_distance(cluster_centroids[0]), training_vecs[i].squared_distance(cluster_centroids[1]), training_vecs[i].squared_distance(cluster_centroids[2])};
    ...
    static bool encode_block_3_subsets(trial_result& res, uint32_t cem, uint32_t grid_w, uint32_t grid_h, uint32_t weights_ise_range, uint32_t endpoints_ise_range, const half_vec3* pBlock_pixels_half, const vec4F* pBlock_pixels_q16,
                                       astc_hdr_codec_base_options& coptions, bool uber_mode_flag, const int* pEst_patterns, int num_est_patterns, uint32_t comp_level, opt_mode_t mode11_opt_mode)
    {
        ...
        for (uint32_t unique_p_iter = 0; unique_p_iter < n; unique_p_iter++)
        {
            ...
            for (uint32_t part_iter = 0; part_iter < NUM_SUBSETS; part_iter++)
            {
                ...
                else
                {
                    ...
                    e += encode_astc_hdr_block_mode_11(part_total_pixels[part_iter], (basist::half_float(*)[3])part_half_pixels[part_iter], (vec4F*)part_pixels_q16[part_iter], best_log_blk.m_weight_ise_range, best_submode[part_iter], BIG_FLOAT_VAL,
                                                       blk_endpoints[part_iter], blk_weights[part_iter], coptions, false, best_log_blk.m_endpoint_ise_range, uber_mode_flag, false, FIRST_MODE11_SUBMODE_INDEX, MAX_MODE11_SUBMODE_INDEX, false,
                                                       mode11_opt_mode);
                }
    ...
    static void code_block(bitwise_coder& coder, const astc_helpers::log_astc_block& log_blk, block_mode block_mode_index, endpoint_mode em, const uint8_t* pEP_deltas)
    {
...
static void filter_block(uint32_t grid_x, uint32_t grid_y, const vec3F* pSrc_block, half_vec3 *pDst_block_half3, vec4F *pDst_block_q16)
{
    ...
    static void create_smooth_maps2(vector2D<float>& smooth_block_mse_scales, const image& orig_img, smooth_map_params& params, image* pUltra_smooth_img = nullptr)
    {
    ...
    struct candidate_encoding
    {
        ...
        candidate_encoding() { clear(); }

        candidate_encoding(const candidate_encoding& other) { *this = other; }

        candidate_encoding(candidate_encoding&& other) { *this = std::move(other); }
    ...
    bool decode_astc_block(uint32_t block_w, uint32_t block_h, astc_helpers::log_astc_block& log_blk, vec3F* pPixels)
    {
        ...
        for (uint32_t y = 0; y < block_h; y++)
        {
            for (uint32_t x = 0; x < block_w; x++)
            {
                pPixels[x + y * block_w].set(basist::half_to_float(decoded_pixels_half4[x + y * block_w][0]), basist::half_to_float(decoded_pixels_half4[x + y * block_w][1]), basist::half_to_float(decoded_pixels_half4[x + y * block_w][2]));
            } // x
        } // y
...
#define SYNC_MARKERS (0)
    ...
    static bool unpack_physical_astc_block_google(const void* pBlock, uint32_t block_width, uint32_t block_height, vec4F* pPixels) { return basisu_astc::astc::decompress_hdr((float*)pPixels, (uint8_t*)pBlock, block_width, block_height); }
    ...
    static void estimate_partitions_mode7_and_11(uint32_t num_parts,                                                                           // 2 or 3 partitions
                                                 ...
                                                 uint32_t num_desired_pats, int* pDesired_pat_indices_mode11, int* pDesired_pat_indices_mode7) // output indices
    {
    ...
    static void estimate_partitions_mode7(uint32_t num_parts,                                                           // 2 or 3 partitions
                                          uint32_t num_unique_pats, const partition_pattern_vec* pUnique_pats,          // list of all unique, canonicalized patterns
                                          uint32_t num_pats_to_examine, const uint32_t* pUnique_pat_indices_to_examine, // indices of pats to examine
                                          const vec3F*                       pHalf_pixels_as_floats,                    // block's half pixel values casted to floats
                                          const astc_hdr_codec_base_options& coptions,                                  // options
    ...
    static float calc_deblocking_penalty_itp(uint32_t bx, uint32_t by, uint32_t width, uint32_t height, const imagef& pass_src_img_itp, const candidate_encoding& candidate)
    {
    ...
    static bool calc_strip_size(float lambda, uint32_t num_blocks_y, uint32_t total_threads, bool force_one_strip, uint32_t& res_total_strips, uint32_t& res_rows_per_strip, astc_hdr_6x6_global_config& global_cfg)
    {
    ...
    struct uastc_hdr_6x6_debug_state
    {
        ...
        void print(uint32_t total_blocks) const
        {
            ...
            for (uint32_t i = 0; i < std::size(m_block_mode_hist); i++)
            {
                ...
                fmt_printf("{ 2}: uses: { 6}, cem: {}, dp: {} chan: {}, parts: {}, grid: {}x{}, endpoint levels: {}, weight levels: {}, Avg bits: {}, Avg Max Std Dev: {}, RG: {} RB: {} GB: {}\n", i, total_uses, bm_desc.m_cem, bm_desc.m_dp,
                           bm_desc.m_dp_channel, bm_desc.m_num_partitions, bm_desc.m_grid_x, bm_desc.m_grid_y, astc_helpers::get_ise_levels(bm_desc.m_endpoint_ise_range), astc_helpers::get_ise_levels(bm_desc.m_weight_ise_range),
                           total_uses ? ((double)m_block_mode_total_bits[i] / total_uses) : 0.0f, avg_std_dev, avg_cross_correlations[0], avg_cross_correlations[1], avg_cross_correlations[2]);
    ...
    static bool compress_strip_task(uint32_t strip_index, uint32_t total_strips, uint32_t strip_first_by, uint32_t strip_last_by, uint32_t num_blocks_x, uint32_t num_blocks_y, uint32_t total_blocks, uint32_t width, uint32_t height,
        ...
        for (uint32_t by = strip_first_by; by <= strip_last_by; by++)
        {
            ...
            for (uint32_t bx = 0; bx < num_blocks_x; bx++)
            {
                ...
                for (uint32_t outer_pass = 0; outer_pass < 3; outer_pass++)
                {
                    ...
                    const imagef& pass_src_img = (outer_pass == 2) ? enc_state.src_img_filtered2 : ((outer_pass == 1) ? enc_state.src_img_filtered1 : enc_state.src_img);

                    const imagef& pass_src_img_itp = (outer_pass == 2) ? enc_state.src_img_filtered2_itp : ((outer_pass == 1) ? enc_state.src_img_filtered1_itp : enc_state.src_img_itp);
                    ...
                    // R vs. G
                    half_cross_chan_stats[0].calc_pearson(NUM_BLOCK_PIXELS, &half_pixels_as_floats[0][0][0], &half_pixels_as_floats[0][0][1], 3, 3, &half_comp_stats[0], &half_comp_stats[1]);

                    // R vs. B
                    half_cross_chan_stats[1].calc_pearson(NUM_BLOCK_PIXELS, &half_pixels_as_floats[0][0][0], &half_pixels_as_floats[0][0][2], 3, 3, &half_comp_stats[0], &half_comp_stats[2]);

                    // G vs. B
                    half_cross_chan_stats[2].calc_pearson(NUM_BLOCK_PIXELS, &half_pixels_as_floats[0][0][1], &half_pixels_as_floats[0][0][2], 3, 3, &half_comp_stats[1], &half_comp_stats[2]);
                    ...
                    if ((!is_solid_block) && (global_cfg.m_lambda > 0.0f))
                    {
                        for (uint32_t reuse_delta_index = 0; reuse_delta_index < global_cfg.m_num_reuse_xy_deltas; reuse_delta_index++)
                        {
                            ...
                            if (prev_coded_log_blk.m_num_partitions == 1)
                            {
                                ...
                                {
                                    status = get_astc_hdr_mode_7_block_colors(coded_log_blk.m_endpoints, &decoded_half[0][0], nullptr, astc_helpers::get_ise_levels(coded_log_blk.m_weight_ise_range), coded_log_blk.m_weight_ise_range,
                                                                              coded_log_blk.m_endpoint_ise_range);
                                }
                                else
                                {
                                    status = get_astc_hdr_mode_11_block_colors(coded_log_blk.m_endpoints, &decoded_half[0][0], nullptr, astc_helpers::get_ise_levels(coded_log_blk.m_weight_ise_range), coded_log_blk.m_weight_ise_range,
                                                                               coded_log_blk.m_endpoint_ise_range);
                                }
                                assert(status);
                                ...
                                if (dual_plane)
                                {
                                    eval_selectors_dual_plane(prev_candidate.m_coded_log_blk.m_color_component_selector, BLOCK_W * BLOCK_H, trial_weights0, trial_weights1, (basist::half_float*)&half_pixels[0][0][0],
                                                              astc_helpers::get_ise_levels(coded_log_blk.m_weight_ise_range), &decoded_half[0][0], coptions, UINT32_MAX);

                                    downsample_ise_weights_dual_plane(coded_log_blk.m_weight_ise_range, coded_log_blk.m_weight_ise_range, BLOCK_W, BLOCK_H, grid_x, grid_y, trial_weights0, trial_weights1, coded_log_blk.m_weights);
                                ...
                                {
                                    eval_selectors(BLOCK_W * BLOCK_H, trial_weights0, coded_log_blk.m_weight_ise_range, (basist::half_float*)&half_pixels[0][0][0], astc_helpers::get_ise_levels(coded_log_blk.m_weight_ise_range), &decoded_half[0][0],
                                                   coptions, UINT32_MAX);

                                    downsample_ise_weights(coded_log_blk.m_weight_ise_range, coded_log_blk.m_weight_ise_range, BLOCK_W, BLOCK_H, grid_x, grid_y, trial_weights0, coded_log_blk.m_weights);
                            ...
                            else if (prev_coded_log_blk.m_num_partitions == 2)
                            {
                                ...
                                for (uint32_t part_index = 0; part_index < 2; part_index++)
                                {
                                    ...
                                    if (prev_coded_log_blk.m_color_endpoint_modes[0] == 7)
                                    {
                                        status = get_astc_hdr_mode_7_block_colors(coded_log_blk.m_endpoints + num_endpoint_vals * part_index, &decoded_half[0][0], nullptr, astc_helpers::get_ise_levels(coded_log_blk.m_weight_ise_range),
                                                                                  coded_log_blk.m_weight_ise_range, coded_log_blk.m_endpoint_ise_range);
                                    }
                                    else
                                    {
                                        status = get_astc_hdr_mode_11_block_colors(coded_log_blk.m_endpoints + num_endpoint_vals * part_index, &decoded_half[0][0], nullptr, astc_helpers::get_ise_levels(coded_log_blk.m_weight_ise_range),
                                                                                   coded_log_blk.m_weight_ise_range, coded_log_blk.m_endpoint_ise_range);
                                    }
                                    assert(status);

                                    eval_selectors(part_total_pixels[part_index], blk_weights[part_index], coded_log_blk.m_weight_ise_range, (basist::half_float*)&part_half_pixels[part_index][0][0],
                                                   astc_helpers::get_ise_levels(coded_log_blk.m_weight_ise_range), &decoded_half[0][0], coptions, UINT32_MAX);
                                ...
                                downsample_ise_weights(coded_log_blk.m_weight_ise_range, coded_log_blk.m_weight_ise_range, BLOCK_W, BLOCK_H, grid_x, grid_y, ise_weights, coded_log_blk.m_weights);
                    ...
                    if (global_cfg.m_use_solid_blocks)
                    {
                        ...
                        for (uint32_t y = 0; y < BLOCK_H; y++)
                        {
                            for (uint32_t x = 0; x < BLOCK_W; x++)
                            {
                                if ((block_pixels[0][0][0] != block_pixels[y][x][0]) || (block_pixels[0][0][1] != block_pixels[y][x][1]) || (block_pixels[0][0][2] != block_pixels[y][x][2]))
                                {
                                    solid_block = false;
                                }
                    ...
                    if ((!is_solid_block) || (!global_cfg.m_use_solid_blocks))
                    {
                        ...
                        if (total_parts2)
                        {
                            if (global_cfg.m_brute_force_partition_matching)
                            {
                                ...
                                if (any_2subset_enabled)
                                {
                                    estimate_partitions_mode7_and_11(2, NUM_UNIQUE_PARTITIONS2, g_partitions2, NUM_UNIQUE_PARTITIONS2, (uint32_t*)candidate_pats2, &half_pixels_as_floats[0][0], coptions, total_parts2, best_parts2_mode11, best_parts2_mode7);
                                }
                            ...
                            else
                            {
                                if (comp_level >= 1)
                                {
                                    ...
                                    {
                                        estimate_partitions_mode7_and_11(2, NUM_UNIQUE_PARTITIONS2, g_partitions2, num_candidate_pats2, (uint32_t*)candidate_pats2, &half_pixels_as_floats[0][0], coptions, total_parts2, best_parts2_mode11,
                                                                         best_parts2_mode7);
                                    }
                                }
                                else
						...
						has_estimated_parts3 = estimate_partition3_6x6((basist::half_float(*)[3])half_pixels, best_parts3, total_parts3);
                            ...
                            if (global_cfg.m_brute_force_partition_matching)
                            {
                                ...
                                estimate_partitions_mode7(3, NUM_UNIQUE_PARTITIONS3, g_partitions3, NUM_UNIQUE_PARTITIONS3, (uint32_t*)candidate_pats3, &half_pixels_as_floats[0][0], coptions, total_parts3, (uint32_t*)best_parts3);
                            ...
                            else
                            {
                                ...
                                {
                                    estimate_partitions_mode7(3, NUM_UNIQUE_PARTITIONS3, g_partitions3, num_candidate_pats3, (uint32_t*)candidate_pats3, &half_pixels_as_floats[0][0], coptions, total_parts3, (uint32_t*)best_parts3);
                                }
                            }
#endif
                        }
                        ...
                        for (uint32_t block_mode_iter = 0; block_mode_iter < (uint32_t)block_mode::cBMTotalModes; block_mode_iter++)
                        {
                            ...
                            if ((!use_single_subset_mode7) && (g_block_mode_descs[block_mode_iter].m_cem == 7) && (g_block_mode_descs[block_mode_iter].m_num_partitions == 1))
                            {
                            ...
                            for (uint32_t endpoint_mode_iter = 0; endpoint_mode_iter < (uint32_t)endpoint_mode::cTotal; endpoint_mode_iter++)
                            {
                                ...
                                switch (em)
                                {
                                    ...
                                    case endpoint_mode::cUseUpper:
                                    {
                                        ...
                                        basist::astc_6x6_hdr::requantize_ise_endpoints(neighbor_md.m_cem, neighbor_blk.m_coded_log_blk.m_endpoint_ise_range, neighbor_blk.m_coded_log_blk.m_endpoints, local_md.m_transcode_endpoint_ise_range,
                                                                                       transcode_endpoints);
                                        ...
                                        if (cem == 7)
                                        {
                                            status = get_astc_hdr_mode_7_block_colors(transcode_endpoints, &decoded_half[0][0], nullptr, astc_helpers::get_ise_levels(local_md.m_weight_ise_range), local_md.m_weight_ise_range,
                                                                                      local_md.m_transcode_endpoint_ise_range);
                                        }
                                        else
                                        {
                                            status = get_astc_hdr_mode_11_block_colors(transcode_endpoints, &decoded_half[0][0], nullptr, astc_helpers::get_ise_levels(local_md.m_weight_ise_range), local_md.m_weight_ise_range,
                                                                                       local_md.m_transcode_endpoint_ise_range);
                                        }
                                        if (!status)
                                            break;
                                        ...
                                        if (dual_plane)
                                        {
                                            eval_selectors_dual_plane(local_md.m_dp_channel, BLOCK_W * BLOCK_H, trial_weights0, trial_weights1, (basist::half_float*)&half_pixels[0][0][0], astc_helpers::get_ise_levels(local_md.m_weight_ise_range),
                                                                      &decoded_half[0][0], coptions, UINT32_MAX);

                                            downsample_ise_weights_dual_plane(local_md.m_weight_ise_range, local_md.m_weight_ise_range, BLOCK_W, BLOCK_H, grid_x, grid_y, trial_weights0, trial_weights1, coded_log_blk.m_weights);
                                        }
                                        else
                                        {
                                            eval_selectors(BLOCK_W * BLOCK_H, trial_weights0, local_md.m_weight_ise_range, (basist::half_float*)&half_pixels[0][0][0], astc_helpers::get_ise_levels(local_md.m_weight_ise_range), &decoded_half[0][0], coptions,
                                                           UINT32_MAX);

                                            downsample_ise_weights(local_md.m_weight_ise_range, local_md.m_weight_ise_range, BLOCK_W, BLOCK_H, grid_x, grid_y, trial_weights0, coded_log_blk.m_weights);
                                        }
                                    ...
                                    case endpoint_mode::cUseUpperDelta:
                                    {
                                        ...
                                        basist::astc_6x6_hdr::requantize_ise_endpoints(neighbor_md.m_cem, neighbor_blk.m_coded_log_blk.m_endpoint_ise_range, neighbor_blk.m_coded_log_blk.m_endpoints, astc_helpers::BISE_256_LEVELS, neighbor_endpoints_ise20);
                                        ...
                                        if (cem == 7)
                                        {
                                            ...
                                            err = encode_astc_hdr_block_mode_7(NUM_BLOCK_PIXELS, (basist::half_float(*)[3])half_pixels, (vec4F*)block_pixels_q16, local_md.m_weight_ise_range, best_submode, BIG_FLOAT_VAL, blk_endpoints, blk_weights0,
                                                                               coptions, local_md.m_endpoint_ise_range, first_submode, last_submode, &enc_block_stats);
                                        }
                                        else
                                        {
                                            int maj_index, submode_index;
                                            ...
                                            if (dual_plane)
                                            {
                                                err = encode_astc_hdr_block_mode_11_dual_plane(NUM_BLOCK_PIXELS, (basist::half_float(*)[3])half_pixels, (vec4F*)block_pixels_q16, local_md.m_dp_channel, local_md.m_weight_ise_range, best_submode,
                                                                                               BIG_FLOAT_VAL, blk_endpoints, blk_weights0, blk_weights1, coptions, false, local_md.m_endpoint_ise_range,
                                                                                               false, // uber_mode_flag,
                                                                                               false, first_submode, last_submode, true);
                                            }
                                            else
                                            {
                                                err = encode_astc_hdr_block_mode_11(NUM_BLOCK_PIXELS, (basist::half_float(*)[3])half_pixels, (vec4F*)block_pixels_q16, local_md.m_weight_ise_range, best_submode, BIG_FLOAT_VAL, blk_endpoints, blk_weights0,
                                                                                    coptions, false, local_md.m_endpoint_ise_range,
                                                                                    false, // uber_mode_flag,
                                                                                    false, first_submode, last_submode, true, mode11_opt_mode, &enc_block_stats);
                                            }
                                        }
                                        ...
                                        if (all_deltas_in_limits)
                                        {
                                            ...
                                            if (dual_plane)
                                            {
                                                downsample_ise_weights_dual_plane(local_md.m_weight_ise_range, local_md.m_weight_ise_range, BLOCK_W, BLOCK_H, grid_x, grid_y, blk_weights0, blk_weights1, coded_log_blk.m_weights);
                                            }
                                            else
                                            {
                                                downsample_ise_weights(local_md.m_weight_ise_range, local_md.m_weight_ise_range, BLOCK_W, BLOCK_H, grid_x, grid_y, blk_weights0, coded_log_blk.m_weights);
                                            }
                                    ...
                                    case endpoint_mode::cRaw:
                                    {
                                        ...
                                        if (mode_desc.m_num_partitions == 3)
                                        {
                                            ...
                                            status = encode_block_3_subsets(res, cem, mode_desc.m_grid_x, mode_desc.m_grid_y, mode_desc.m_weight_ise_range, mode_desc.m_endpoint_ise_range, &half_pixels[0][0], (vec4F*)block_pixels_q16, coptions,
                                                                            uber_mode_flag, best_parts3, total_parts3, comp_level, mode11_opt_mode);
                                        ...
                                        else if (mode_desc.m_num_partitions == 2)
                                        {
                                            ...
                                            for (uint32_t est_part_iter = 0; est_part_iter < total_parts2; est_part_iter++)
                                            {
                                                ...
                                                status = encode_block_2_subsets(results, mode_desc.m_grid_x, mode_desc.m_grid_y, mode_desc.m_cem, mode_desc.m_weight_ise_range, mode_desc.m_endpoint_ise_range, &half_pixels[0][0], (vec4F*)block_pixels_q16,
                                                                                coptions, uber_mode_flag, (cem == 11) ? best_parts2_mode11[est_part_iter] : best_parts2_mode7[est_part_iter], comp_level, mode11_opt_mode, true);
                                        ...
                                        else
                                        {
                                            ...
                                            {
                                                double e = encode_astc_hdr_block_downsampled_mode_11(BLOCK_W, BLOCK_H, grid_x, grid_y, mode_desc.m_weight_ise_range, mode_desc.m_endpoint_ise_range, NUM_BLOCK_PIXELS,
                                                                                                     (basist::half_float(*)[3])pBlock_pixels_half, pBlock_pixels_q16, BIG_FLOAT_VAL, FIRST_MODE11_SUBMODE_INDEX, MAX_MODE11_SUBMODE_INDEX, false,
                                                                                                     mode11_opt_mode, coded_log_blk.m_endpoints, coded_log_blk.m_weights, best_submode, coptions, &enc_block_stats);
                                            ...
                                            else
                                            {
                                                if (cem == 7)
                                                {
                                                    ...
                                                    double e = encode_astc_hdr_block_mode_7(NUM_BLOCK_PIXELS, (basist::half_float(*)[3])pBlock_pixels_half, pBlock_pixels_q16, mode_desc.m_weight_ise_range, best_submode, BIG_FLOAT_VAL,
                                                                                            coded_log_blk.m_endpoints, blk_weights0, coptions, mode_desc.m_endpoint_ise_range, 0, MAX_MODE7_SUBMODE_INDEX, &enc_block_stats);
                                                ...
                                                else
                                                {
                                                    ...
                                                    if (dual_plane)
                                                    {
                                                        e = encode_astc_hdr_block_mode_11_dual_plane(NUM_BLOCK_PIXELS, (basist::half_float(*)[3])pBlock_pixels_half, pBlock_pixels_q16, mode_desc.m_dp_channel, mode_desc.m_weight_ise_range, best_submode,
                                                                                                     BIG_FLOAT_VAL, coded_log_blk.m_endpoints, blk_weights0, blk_weights1, coptions, false, mode_desc.m_endpoint_ise_range, uber_mode_flag, false, -1, 7,
                                                                                                     false);
                                                    }
                                                    else
                                                    {
                                                        e = encode_astc_hdr_block_mode_11(NUM_BLOCK_PIXELS, (basist::half_float(*)[3])pBlock_pixels_half, pBlock_pixels_q16, mode_desc.m_weight_ise_range, best_submode, BIG_FLOAT_VAL,
                                                                                          coded_log_blk.m_endpoints, blk_weights0, coptions, false, mode_desc.m_endpoint_ise_range, uber_mode_flag, false, -1, 7, false, mode11_opt_mode, &enc_block_stats);
                                                    }
                                                ...
                                                if (dual_plane)
                                                {
                                                    downsample_ise_weights_dual_plane(mode_desc.m_weight_ise_range, mode_desc.m_weight_ise_range, BLOCK_W, BLOCK_H, grid_x, grid_y, blk_weights0, blk_weights1, coded_log_blk.m_weights);
                                                }
                                                else
                                                {
                                                    downsample_ise_weights(mode_desc.m_weight_ise_range, mode_desc.m_weight_ise_range, BLOCK_W, BLOCK_H, grid_x, grid_y, blk_weights0, coded_log_blk.m_weights);

                                                    if ((comp_level >= MIN_REFINE_LEVEL) && ((grid_x < BLOCK_W) || (grid_y < BLOCK_H)))
                                                    {
                                                        bool refine_status = refine_endpoints(cem, mode_desc.m_endpoint_ise_range, coded_log_blk.m_endpoints, 6, 6, mode_desc.m_grid_x, mode_desc.m_grid_y, coded_log_blk.m_weights,
                                                                                              mode_desc.m_weight_ise_range, BLOCK_W * BLOCK_H, (basist::half_float(*)[3])pBlock_pixels_half, (vec4F*)pBlock_pixels_q16, nullptr, coptions, mode11_opt_mode);
                                                        BASISU_NOTE_UNUSED(refine_status);
                                                    }
                                                }
                                            }
                    ...
                    if (global_cfg.m_lambda == 0.0f)
                    {
                        ...
                        if (global_cfg.m_gaussian2_fallback && (comp_level >= 1) && (outer_pass == 1) && (very_complex_block) && (best_d_ssim > SWITCH_TO_GAUSSIAN_FILTERED_THRESH2_D_SSIM) && (block_hy >= 18.0f) && (block_y_contrast_ratio > 150.0f) &&
                            (block_avg_y >= 1.5f))
                        {
                    ...
                    if ((global_cfg.m_debug_images) && ((best_candidate.m_encoding_type != encoding_type::cRun) && (best_candidate.m_encoding_type != encoding_type::cSolid)))
                    {
                        ...
                        {
                            case encoding_type::cRun: c.set(0, 0, 0, 255); break;
                            case encoding_type::cSolid:
    ...
    bool compress_photo(const basisu::imagef& orig_src_img, const astc_hdr_6x6_global_config& orig_global_cfg, job_pool* pJob_pool, basisu::uint8_vec& intermediate_tex_data, basisu::uint8_vec& astc_tex_data, result_metrics& metrics)
    {
        ...
        for (uint32_t strip_index = 0; strip_index < total_strips; strip_index++)
        {
            ...
            pJob_pool->add_job(
                [&any_failed_flag, &global_cfg, &debug_state, &enc_state, strip_index, total_strips, strip_first_by, strip_last_by, num_blocks_x, num_blocks_y, total_blocks, width, height]
                {
                    if (!any_failed_flag)
                    {
                        bool status = compress_strip_task(strip_index, total_strips, strip_first_by, strip_last_by, num_blocks_x, num_blocks_y, total_blocks, width, height, global_cfg, debug_state, enc_state);
        ...
        if ((enc_state.final_astc_blocks.get_width() != decoded_blocks1.get_width()) || (enc_state.final_astc_blocks.get_height() != decoded_blocks1.get_height()))
        {
            fmt_error_printf("Decode size mismatch with decode_file\n");
            return false;
        }

        if ((enc_state.final_astc_blocks.get_width() != decoded_blocks2.get_width()) || (enc_state.final_astc_blocks.get_height() != decoded_blocks2.get_height()))
        {
            ...
            if (write_astc_file((global_cfg.m_output_image_prefix + "decoded.astc").c_str(), decoded_blocks1.get_ptr(), BLOCK_W, BLOCK_H, width, height))
            {
                ...
                if (read_file_to_vec((global_cfg.m_output_image_prefix + "decoded.astc").c_str(), astc_file_data))
                {
                    if (astc_file_data.size() > 16)
                    {
                        ...
                        if (global_cfg.m_debug_output)
                        {
                            fmt_printf(".ASTC file size (less header): {}, bits/pixel: {}, Deflate bits/pixel: {}\n", (uint64_t)astc_file_data.size(), (float)astc_file_data.size() * 8.0f / (float)(width * height),
