...
#include "ctff/private/encoder/jpgd.h"
...
#endif
#include "ctff/private/3rdparty/tinyexr.h"
...
#include "ctff/private/3rdparty/qoi.h"
...
namespace basisu
{
    ...
    uint8_t g_hamming_dist[256] = {0, 1, 1, 2, 1, 2, 2, 3, 1, 2, 2, 3, 2, 3, 3, 4, 1, 2, 2, 3, 2, 3, 3, 4, 2, 3, 3, 4, 3, 4, 4, 5, 1, 2, 2, 3, 2, 3, 3, 4, 2, 3, 3, 4, 3, 4, 4, 5, 2, 3, 3, 4, 3, 4, 4, 5, 3, 4, 4, 5, 4, 5, 5, 6,
...
#if defined(_WIN32)
    void platform_sleep(uint32_t ms) { Sleep(ms); }
#else
...
#if defined(_WIN32)
    inline void query_counter(timer_ticks* pTicks) { QueryPerformanceCounter(reinterpret_cast<LARGE_INTEGER*>(pTicks)); }
    inline void query_counter_frequency(timer_ticks* pTicks) { QueryPerformanceFrequency(reinterpret_cast<LARGE_INTEGER*>(pTicks)); }
...
#    include <sys/time.h>
    ...
    inline void query_counter_frequency(timer_ticks* pTicks) { *pTicks = 1000000; }
...
#    error TODO
    ...
    bool image_resample(const image& src, image& dst, bool srgb, const char* pFilter, float filter_scale, bool wrapping, uint32_t first_comp, uint32_t num_comps)
    {
        ...
        resamplers[0] = new Resampler(src_w, src_h, dst_w, dst_h, wrapping ? Resampler::BOUNDARY_WRAP : Resampler::BOUNDARY_CLAMP, 0.0f, 1.0f, pFilter, nullptr, nullptr, filter_scale, filter_scale, 0, 0);
        samples[0].resize(src_w);

        for (uint32_t i = 1; i < num_comps; ++i)
        {
            resamplers[i] = new Resampler(src_w, src_h, dst_w, dst_h, wrapping ? Resampler::BOUNDARY_WRAP : Resampler::BOUNDARY_CLAMP, 0.0f, 1.0f, pFilter, resamplers[0]->get_clist_x(), resamplers[0]->get_clist_y(), filter_scale, filter_scale, 0, 0);
    ...
    bool image_resample(const imagef& src, imagef& dst, const char* pFilter, float filter_scale, bool wrapping, uint32_t first_comp, uint32_t num_comps)
    {
        assert((first_comp + num_comps) <= 4);
        ...
        resamplers[0] = new Resampler(src_w, src_h, dst_w, dst_h, wrapping ? Resampler::BOUNDARY_WRAP : Resampler::BOUNDARY_CLAMP, 1.0f, 0.0f, // no clamping
                                      pFilter, nullptr, nullptr, filter_scale, filter_scale, 0, 0);
        samples[0].resize(src_w);

        for (uint32_t i = 1; i < num_comps; ++i)
        {
            resamplers[i] = new Resampler(src_w, src_h, dst_w, dst_h, wrapping ? Resampler::BOUNDARY_WRAP : Resampler::BOUNDARY_CLAMP, 1.0f, 0.0f, // no clamping
...
#pragma pack(pop)
    ...
    bool write_exr(const char* pFilename, const imagef& img, uint32_t n_chans, uint32_t flags)
    {
        ...
        const bool linear_hint = (flags & WRITE_EXR_LINEAR_HINT) != 0, store_float = (flags & WRITE_EXR_STORE_FLOATS) != 0, no_compression = (flags & WRITE_EXR_NO_COMPRESSION) != 0;
