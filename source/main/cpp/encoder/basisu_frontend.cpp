...
#define BASISU_FRONTEND_VERIFY(c)            \
    ...
    {                                        \
        if (!(c))                            \
            handle_verify_failure(__LINE__); \
    } while (0)
...
namespace basisu
{
    ...
    bool basisu_frontend::init(const params& p)
    {
        debug_printf("basisu_frontend::init: Multithreaded: %u, Job pool total threads: %u, NumEndpointClusters: %u, NumSelectorClusters: %u, Perceptual: %u, CompressionLevel: %u\n", p.m_multithreaded,
                     p.m_pJob_pool ? p.m_pJob_pool->get_total_threads() : 0, p.m_max_endpoint_clusters, p.m_max_selector_clusters, p.m_perceptual, p.m_compression_level);
        ...
        debug_printf("Endpoint refinement: %u, Hierarchical endpoint codebooks: %u, Hierarchical selector codebooks: %u, Endpoint codebook iters: %u, Selector codebook iters: %u\n", m_endpoint_refinement, m_use_hierarchical_endpoint_codebooks,
                     m_use_hierarchical_selector_codebooks, m_num_endpoint_codebook_iterations, m_num_selector_codebook_iterations);
        ...
        for (uint32_t pass = 0; pass < NUM_PASSES; pass++)
        {
            ...
            for (uint32_t block_index_iter = 0; block_index_iter < m_total_blocks; block_index_iter += N)
            {
                ...
                m_params.m_pJob_pool->add_job(
                    [this, first_index, last_index, pass]
                    {
                        for (uint32_t block_index = first_index; block_index < last_index; block_index++)
                        {
    ...
    void basisu_frontend::init_endpoint_training_vectors()
    {
        ...
        for (uint32_t block_index_iter = 0; block_index_iter < m_total_blocks; block_index_iter += N)
        {
            ...
            m_params.m_pJob_pool->add_job(
                [this, first_index, last_index, &training_vecs]
                {
                    for (uint32_t block_index = first_index; block_index < last_index; block_index++)
                    {
                        const etc_block& blk = m_etc1_blocks_etc1s[block_index];
    ...
    void basisu_frontend::generate_endpoint_clusters()
    {
        ...
        bool status = generate_hierarchical_codebook_threaded(m_endpoint_clusterizer, m_params.m_max_endpoint_clusters, m_use_hierarchical_endpoint_codebooks ? parent_codebook_size : 0, m_endpoint_clusters, m_endpoint_parent_clusters, max_threads,
                                                              m_params.m_pJob_pool, true);
    ...
    void basisu_frontend::generate_endpoint_codebook(uint32_t step)
    {
        ...
        if (m_params.m_pOpenCL_context && !step)
        {
            ...
            if (opencl_encode_etc1s_pixel_clusters(m_params.m_pOpenCL_context, output_blocks.data(), total_clusters, sorted_pixel_clusters.data(), input_pixels.size(), input_pixels.data(), pixel_weights.data(), m_params.m_perceptual, total_perms))
            {
        ...
        if (use_cpu)
        {
            ...
            for (uint32_t cluster_index_iter = 0; cluster_index_iter < m_endpoint_clusters.size(); cluster_index_iter += N)
            {
                ...
                m_params.m_pJob_pool->add_job(
                    [this, first_index, last_index, step]
                    {
                        for (uint32_t cluster_index = first_index; cluster_index < last_index; cluster_index++)
                        {
                            const basisu::vector<uint32_t>& cluster_indices = m_endpoint_clusters[cluster_index];
    ...
    uint32_t basisu_frontend::refine_endpoint_clusterization()
    {
        ...
        if (m_params.m_pOpenCL_context && m_use_hierarchical_endpoint_codebooks)
        {
            ...
            bool status = opencl_refine_endpoint_clusterization(m_params.m_pOpenCL_context, cl_block_info_structs.data(), total_actual_endpoint_clusters, cl_endpoint_cluster_structs.data(), sorted_block_indices.data(), best_cluster_indices.data(),
        ...
        if (use_cpu)
        {
            ...
            for (uint32_t block_index_iter = 0; block_index_iter < m_total_blocks; block_index_iter += N)
            {
                ...
                m_params.m_pJob_pool->add_job(
                    [this, first_index, last_index, &best_cluster_indices, &block_clusters]
                    {
                        for (uint32_t block_index = first_index; block_index < last_index; block_index++)
                        {
...
#    endif
                            ...
                            skip_cluster:
                                if ((total_err < best_cluster_err) || ((cluster_iter == cluster_index) && (total_err == best_cluster_err)))
                                {
    ...
    void basisu_frontend::create_initial_packed_texture()
    {
        ...
        if ((m_params.m_pOpenCL_context) && (opencl_is_available()))
        {
            ...
            bool status = opencl_determine_selectors(m_params.m_pOpenCL_context, block_etc5_color_intens.data(), m_encoded_blocks.data(), m_params.m_perceptual);
            if (!status)
            {
    ...
    void basisu_frontend::generate_selector_clusters()
    {
        ...
        bool status = generate_hierarchical_codebook_threaded(selector_clusterizer, m_params.m_max_selector_clusters, m_use_hierarchical_selector_codebooks ? parent_codebook_size : 0, m_selector_cluster_block_indices,
                                                              m_selector_parent_cluster_block_indices, max_threads, m_params.m_pJob_pool, false);
        BASISU_FRONTEND_VERIFY(status);
    ...
    void basisu_frontend::find_optimal_selector_clusters_for_each_block()
    {
        ...
        if ((m_params.m_pOpenCL_context) && m_use_hierarchical_selector_codebooks)
        {
            ...
            bool status = opencl_find_optimal_selector_clusters_for_each_block(m_params.m_pOpenCL_context, block_structs.data(), total_input_selectors, selector_structs.data(), selector_cluster_indices.data(), output_selector_cluster_indices.data(),
                                                                               m_params.m_perceptual);
        ...
        if (use_cpu)
        {
            ...
            for (uint32_t block_index_iter = 0; block_index_iter < m_total_blocks; block_index_iter += N)
            {
                ...
                m_params.m_pJob_pool->add_job(
                    [this, first_index, last_index, &unpacked_optimized_cluster_selectors]
                    {
    ...
    void basisu_frontend::reoptimize_remapped_endpoints(const uint_vec& new_block_endpoints, int_vec& old_to_new_endpoint_cluster_indices, bool optimize_final_codebook, uint_vec* pBlock_selector_indices)
    {
        ...
        for (uint32_t cluster_index_iter = 0; cluster_index_iter < new_endpoint_cluster_block_indices.size(); cluster_index_iter += N)
        {
            ...
            m_params.m_pJob_pool->add_job(
                [this, first_index, last_index, &cluster_improved, &cluster_valid, &new_endpoint_cluster_block_indices, &pBlock_selector_indices]
                {
                    for (uint32_t cluster_index = first_index; cluster_index < last_index; cluster_index++)
                    {
