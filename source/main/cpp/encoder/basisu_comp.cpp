...
#define PRINT_FLOAT_VALUE(v) fmt_debug_printf("{}: {} {}\n", BASISU_STRINGIZE2(v), static_cast<float>(m_params.v), m_params.v.was_changed());
            ...
            fmt_debug_printf("Source LDR images: {}, HDR images: {}, filenames: {}, alpha filenames: {}, LDR mipmap images: {}, HDR mipmap images: {}\n", (uint64_t)m_params.m_source_images.size(), (uint64_t)m_params.m_source_images_hdr.size(),
                             (uint64_t)m_params.m_source_filenames.size(), (uint64_t)m_params.m_source_alpha_filenames.size(), (uint64_t)m_params.m_source_mipmap_images.size(), (uint64_t)m_params.m_source_mipmap_images_hdr.size());
            ...
            debug_printf("swizzle: %d,%d,%d,%d\n", m_params.m_swizzle[0], m_params.m_swizzle[1], m_params.m_swizzle[2], m_params.m_swizzle[3]);
...
#undef PRINT_FLOAT_VALUE
    ...
    void basis_compressor::pick_format_mode()
    {
        ...
        if (m_params.m_hdr)
        {
            ...
            switch (m_params.m_hdr_mode)
            {
                case hdr_modes::cUASTC_HDR_4X4: m_fmt_mode = basist::basis_tex_format::cUASTC_HDR_4x4; break;
                case hdr_modes::cASTC_HDR_6X6: m_fmt_mode = basist::basis_tex_format::cASTC_HDR_6x6; break;
                case hdr_modes::cASTC_HDR_6X6_INTERMEDIATE: m_fmt_mode = basist::basis_tex_format::cASTC_HDR_6x6_INTERMEDIATE; break;
        ...
        if (m_params.m_debug)
        {
            switch (m_fmt_mode)
            {
                case basist::basis_tex_format::cETC1S: fmt_debug_printf("Format Mode: cETC1S\n"); break;
                case basist::basis_tex_format::cUASTC4x4: fmt_debug_printf("Format Mode: cUASTC4x4\n"); break;
                case basist::basis_tex_format::cUASTC_HDR_4x4: fmt_debug_printf("Format Mode: cUASTC_HDR_4x4\n"); break;
                case basist::basis_tex_format::cASTC_HDR_6x6: fmt_debug_printf("Format Mode: cASTC_HDR_6x6\n"); break;
                case basist::basis_tex_format::cASTC_HDR_6x6_INTERMEDIATE: fmt_debug_printf("Format Mode: cASTC_HDR_6x6_INTERMEDIATE\n"); break;
                default: assert(0); break;
            }
        }
    }
            ...
            else
            {
                ...
                if (m_params.m_status_output)
                {
                    fmt_printf("Mode: ASTC 6x6 HDR {}, Base Level: {}, Highest Level: {}, Lambda: {}, REC 2020: {}\n", (m_params.m_hdr_mode == hdr_modes::cASTC_HDR_6X6_INTERMEDIATE) ? "Intermediate" : "",
                               m_params.m_astc_hdr_6x6_options.m_master_comp_level, m_params.m_astc_hdr_6x6_options.m_highest_comp_level, m_params.m_astc_hdr_6x6_options.m_lambda, m_params.m_astc_hdr_6x6_options.m_rec2020_bt2100_color_gamut);
                }
...
#undef COMP
            ...
            bool operator!=(const uastc_blk_desc& desc) const { return !(*this == desc); }
        };
        ...
        for (uint32_t slice_index = 0; slice_index < m_slice_descs.size(); slice_index++)
        {
            ...
            for (uint32_t block_index_iter = 0; block_index_iter < total_blocks; block_index_iter += N)
            {
                ...
                m_params.m_pJob_pool->add_job(
                    [this, first_index, last_index, num_blocks_x, num_blocks_y, total_blocks, &source_image, &tex, &total_blocks_processed, &any_failures, &enc_stats, &unique_block_descs, &unique_block_desc_mutex]
                    {
        ...
        if (m_params.m_debug)
        {
            ...
            {
                debug_printf("%u. Total uses: %u %3.2f%%, solid color: %u\n", c, it->second.m_count, ((float)it->second.m_count * 100.0f) / enc_stats.m_total_blocks, it->first.m_solid_flag);
    ...
    basis_compressor::error_code basis_compressor::encode_slices_to_uastc_4x4_ldr()
    {
        ...
        for (uint32_t slice_index = 0; slice_index < m_slice_descs.size(); slice_index++)
        {
            ...
            for (uint32_t block_index_iter = 0; block_index_iter < total_blocks; block_index_iter += N)
            {
                ...
                m_params.m_pJob_pool->add_job(
                    [this, first_index, last_index, num_blocks_x, num_blocks_y, total_blocks, &source_image, &tex, &total_blocks_processed]
                    {
                        BASISU_NOTE_UNUSED(num_blocks_y);
            ...
            if (m_params.m_rdo_uastc_ldr_4x4)
            {
                ...
                bool status = uastc_rdo(tex.get_total_blocks(), (basist::uastc_block*)tex.get_ptr(), (const color_rgba*)m_source_blocks[slice_desc.m_first_block_index].m_pixels, rdo_params, m_params.m_pack_uastc_ldr_4x4_flags,
                                        m_params.m_rdo_uastc_ldr_4x4_multithreading ? m_params.m_pJob_pool : nullptr,
    ...
    bool basis_compressor::generate_mipmaps(const image& img, basisu::vector<image>& mips, bool has_alpha)
    {
        ...
        for (uint32_t level = 1; level < total_levels; level++)
        {
            ...
            int result = stbir_resize_uint8_generic((const uint8_t*)img.get_ptr(), img.get_width(), img.get_height(), img.get_pitch() * sizeof(color_rgba), (uint8_t*)level_img.get_ptr(), level_img.get_width(), level_img.get_height(),
                                                    level_img.get_pitch() * sizeof(color_rgba), has_alpha ? 4 : 3, has_alpha ? 3 : STBIR_ALPHA_CHANNEL_NONE, m_params.m_mip_premultiplied ? STBIR_FLAG_ALPHA_PREMULTIPLIED : 0,
                                                    m_params.m_mip_wrapping ? STBIR_EDGE_WRAP : STBIR_EDGE_CLAMP, filter, m_params.m_mip_srgb ? STBIR_COLORSPACE_SRGB : STBIR_COLORSPACE_LINEAR, nullptr);
    ...
    void basis_compressor::clean_hdr_image(imagef& src_img)
    {
        ...
        if (max_used_val > basist::ASTC_HDR_MAX_VAL)
        {
            ...
            printf("Warning: The input HDR image's maximum used float value was %f, which is too high to encode as ASTC HDR. The image's components have been linearly scaled so the maximum used value is %f, by multiplying by %f.\n", max_used_val,
                   basist::ASTC_HDR_MAX_VAL, inv_hdr_image_scale);
    ...
    bool basis_compressor::read_dds_source_images()
    {
        ...
        for (uint32_t i = 0; i < m_params.m_source_filenames.size(); i++)
        {
            ...
            if (m_params.m_status_output)
            {
                printf("Read DDS file \"%s\", %s, %ux%u, %zu mipmap levels\n", m_params.m_source_filenames[i].c_str(), ldr_mips.size() ? "LDR" : "HDR", ldr_mips.size() ? ldr_mips[0].get_width() : hdr_mips[0].get_width(),
                       ldr_mips.size() ? ldr_mips[0].get_height() : hdr_mips[0].get_height(), ldr_mips.size() ? ldr_mips.size() : hdr_mips.size());
            }
    ...
    bool basis_compressor::read_source_images()
    {
        ...
        const uint32_t total_source_files = m_params.m_read_source_images ? (uint32_t)m_params.m_source_filenames.size() : (m_params.m_hdr ? (uint32_t)m_params.m_source_images_hdr.size() : (uint32_t)m_params.m_source_images.size());
        ...
        for (uint32_t source_file_index = 0; source_file_index < total_source_files; source_file_index++)
        {
            ...
            if (m_params.m_swizzle[0] != 0 || m_params.m_swizzle[1] != 1 || m_params.m_swizzle[2] != 2 || m_params.m_swizzle[3] != 3)
            {
                if (!m_params.m_hdr)
                {
        ...
        for (uint32_t source_file_index = 0; source_file_index < total_source_files; source_file_index++)
        {
            ...
            for (uint32_t slice_index = 0; slice_index < (m_params.m_hdr ? slices_hdr.size() : slices.size()); slice_index++)
            {
                ...
                debug_printf("****** Slice %u: mip %u, alpha_slice: %u, filename: \"%s\", original: %ux%u actual: %ux%u\n", m_slice_descs.size() - 1, mip_indices[slice_index], is_alpha_slice, source_filename.c_str(), orig_width, orig_height,
                             m_params.m_hdr ? pSlice_image_hdr->get_width() : pSlice_image->get_width(), m_params.m_hdr ? pSlice_image_hdr->get_height() : pSlice_image->get_height());
        ...
        for (uint32_t i = 0; i < m_slice_descs.size(); i++)
        {
            ...
            if (m_params.m_status_output)
            {
                printf("Slice: %u, alpha: %u, orig width/height: %ux%u, width/height: %ux%u, first_block: %u, image_index: %u, mip_level: %u, iframe: %u\n", i, slice_desc.m_alpha, slice_desc.m_orig_width, slice_desc.m_orig_height, slice_desc.m_width,
                       slice_desc.m_height, slice_desc.m_first_block_index, slice_desc.m_source_file_index, slice_desc.m_mip_index, slice_desc.m_iframe);
            }
    ...
    bool basis_compressor::create_basis_file_and_transcode()
    {
        ...
        if (m_params.m_validate_output_data)
        {
            ...
            for (uint32_t slice_iter = 0; slice_iter < m_slice_descs.size(); slice_iter++)
            {
                ...
                decoded_texture.init(tex_format, m_slice_descs[slice_iter].m_width, m_slice_descs[slice_iter].m_height);
                ...
                if (!decoder.transcode_slice(&comp_data[0], (uint32_t)comp_data.size(), slice_iter, reinterpret_cast<etc_block*>(decoded_texture.get_ptr()), total_dst_blocks, blk_format, bytes_per_block))
                {
            ...
            if (m_params.m_hdr)
            {
                if (is_hdr_6x6)
                {
                    ...
                    for (uint32_t i = 0; i < m_slice_descs.size(); i++)
                    {
                        ...
                        if (!decoder.transcode_slice(&comp_data[0], (uint32_t)comp_data.size(), i, reinterpret_cast<basist::astc_blk*>(decoded_texture.get_ptr()), m_slice_descs[i].m_num_blocks_x * m_slice_descs[i].m_num_blocks_y,
                                                     basist::block_format::cASTC_HDR_6x6, 16))
                        {
                ...
                else
                {
                    ...
                    for (uint32_t i = 0; i < m_slice_descs.size(); i++)
                    {
                        ...
                        if (!decoder.transcode_slice(&comp_data[0], (uint32_t)comp_data.size(), i, reinterpret_cast<basist::astc_blk*>(decoded_texture.get_ptr()), m_slice_descs[i].m_num_blocks_x * m_slice_descs[i].m_num_blocks_y,
                                                     basist::block_format::cASTC_HDR_4x4, 16))
                        {
                            error_printf("Transcoding failed to ASTC HDR on slice %u!\n", i);
                            return false;
                        }
            ...
            else
            {
                    ...
                    basist::basis_is_format_supported(basist::transcoder_texture_format::cTFBC7_RGBA, basist::basis_tex_format::cETC1S))
                {
                    for (uint32_t i = 0; i < m_slice_descs.size(); i++)
                    {
                        ...
                        if (!decoder.transcode_slice(&comp_data[0], (uint32_t)comp_data.size(), i, reinterpret_cast<etc_block*>(decoded_texture.get_ptr()), m_slice_descs[i].m_num_blocks_x * m_slice_descs[i].m_num_blocks_y, basist::block_format::cBC7, 16))
                        {
            ...
            debug_printf("Transcoded to %s in %3.3fms, %f texels/sec\n", m_params.m_hdr ? "BC6H" : (m_params.m_uastc ? "ASTC" : "ETC1"), total_time_etc1s_or_astc * 1000.0f, total_orig_pixels / total_time_etc1s_or_astc);
    ...
    bool basis_compressor::write_output_files_and_compute_stats()
    {
        ...
        if ((m_params.m_compute_stats) && (m_params.m_uastc) && (comp_data.size()))
        {
            ...
            fmt_debug_printf("Output file size: {}, {3.2} bits/texel, LZ compressed file size: {}, {3.2} bits/texel\n", (uint64_t)comp_data.size(), ((float)comp_data.size() * 8.0f) / total_texels, (uint64_t)comp_size, m_basis_bits_per_texel);
        }
    ...
    static uint8_t g_ktx2_etc1s_nonalpha_dfd[44] = {0x2C, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x2,  0x0, 0x28, 0x0, 0xA3, 0x1, 0x2, 0x0, 0x3, 0x3, 0x0,  0x0,  0x0,  0x0,
                                                    0x0,  0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x3F, 0x0, 0x0,  0x0, 0x0,  0x0, 0x0, 0x0, 0x0, 0x0, 0xFF, 0xFF, 0xFF, 0xFF};
    static uint8_t g_ktx2_etc1s_alpha_dfd[60]    = {0x3C, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x2, 0x0, 0x38, 0x0,  0xA3, 0x1,  0x2,  0x0, 0x3,  0x3, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,  0x0,  0x0,  0x0,
                                                    0x3F, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0xFF, 0xFF, 0xFF, 0xFF, 0x40, 0x0, 0x3F, 0xF, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0xFF, 0xFF, 0xFF, 0xFF};

    // colorModel=KTX2_KDF_DF_MODEL_UASTC_LDR_4X4 (0xA6)
    // LDR UASTC 4x4 texture data in a custom block format
    static uint8_t g_ktx2_uastc_ldr_4x4_nonalpha_dfd[44] = {0x2C, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x2,  0x0, 0x28, 0x0, 0xA6, 0x1, 0x2, 0x0, 0x3, 0x3, 0x0,  0x0,  0x10, 0x0,
                                                            0x0,  0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x7F, 0x4, 0x0,  0x0, 0x0,  0x0, 0x0, 0x0, 0x0, 0x0, 0xFF, 0xFF, 0xFF, 0xFF};
    static uint8_t g_ktx2_uastc_ldr_4x4_alpha_dfd[44]    = {0x2C, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x2,  0x0, 0x28, 0x0, 0xA6, 0x1, 0x2, 0x0, 0x3, 0x3, 0x0,  0x0,  0x10, 0x0,
                                                            0x0,  0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x7F, 0x3, 0x0,  0x0, 0x0,  0x0, 0x0, 0x0, 0x0, 0x0, 0xFF, 0xFF, 0xFF, 0xFF};
    ...
    bool basis_compressor::create_ktx2_file()
    {
        ...
        if ((can_use_zstd) && (header.m_supercompression_scheme == basist::KTX2_SS_ZSTANDARD))
        {
            ...
            for (uint32_t level_index = 0; level_index < total_levels; level_index++)
            {
                ...
                size_t result =
                    ZSTD_compress(compressed_level_data_bytes[level_index].data(), compressed_level_data_bytes[level_index].size(), level_data_bytes[level_index].data(), level_data_bytes[level_index].size(), m_params.m_ktx2_zstd_supercompression_level);
    ...
    bool basis_parallel_compress(uint32_t total_threads, const basisu::vector<basis_compressor_params>& params_vec, basisu::vector<parallel_results>& results_vec)
    {
        assert(g_library_initialized);
        if (!g_library_initialized)
        {
        ...
        {
            jpool.add_job(
                [pindex, &params_vec, &results_vec, &result, &opencl_failed]
                {
    ...
    static void* basis_compress(basist::basis_tex_format mode, const basisu::vector<image>* pSource_images, const basisu::vector<imagef>* pSource_images_hdr, uint32_t flags_and_quality, float uastc_rdo_quality, size_t* pSize, image_stats* pStats)
    {
    ...
    void* basis_compress(basist::basis_tex_format mode, const basisu::vector<image>& source_images, uint32_t flags_and_quality, float uastc_rdo_quality, size_t* pSize, image_stats* pStats)
    {
        return basis_compress(mode, &source_images, nullptr, flags_and_quality, uastc_rdo_quality, pSize, pStats);
    }

    void* basis_compress(basist::basis_tex_format mode, const basisu::vector<imagef>& source_images_hdr, uint32_t flags_and_quality, float lambda, size_t* pSize, image_stats* pStats)
    {
        return basis_compress(mode, nullptr, &source_images_hdr, flags_and_quality, lambda, pSize, pStats);
    }

    void* basis_compress(basist::basis_tex_format mode, const uint8_t* pImageRGBA, uint32_t width, uint32_t height, uint32_t pitch_in_pixels, uint32_t flags_and_quality, float uastc_rdo_quality, size_t* pSize, image_stats* pStats)
    {
        if (!pitch_in_pixels)
    ...
    void basis_free_data(void* p) { free(p); }
    ...
    bool basis_benchmark_etc1s_opencl(bool* pOpenCL_failed)
    {
        ...
        {
            tm.start();
            void*  pComp_data = basis_compress(basist::basis_tex_format::cETC1S, images, flags_and_quality, 1.0f, &comp_size, &stats);
        ...
        {
            tm.start();
            void* pComp_data = basis_compress(basist::basis_tex_format::cETC1S, images, flags_and_quality | cFlagUseOpenCL, 1.0f, &comp_size, &stats);
