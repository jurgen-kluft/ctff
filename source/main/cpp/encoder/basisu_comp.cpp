...
#define PRINT_FLOAT_VALUE(v) debug_printf("%s: %f %u\n", BASISU_STRINGIZE2(v), static_cast<float>(m_params.v), m_params.v.was_changed());
            ...
            debug_printf("Source LDR images: %u, HDR images: %u, filenames: %u, alpha filenames: %i, LDR mipmap images: %u, HDR mipmap images: %u\n", m_params.m_source_images.size(), m_params.m_source_images_hdr.size(), m_params.m_source_filenames.size(),
                         m_params.m_source_alpha_filenames.size(), m_params.m_source_mipmap_images.size(), m_params.m_source_mipmap_images_hdr.size());
...
#undef COMP
    ...
    basis_compressor::error_code basis_compressor::encode_slices_to_uastc()
    {
        ...
        for (uint32_t slice_index = 0; slice_index < m_slice_descs.size(); slice_index++)
        {
            ...
            if (m_params.m_rdo_uastc)
            {
                ...
                bool status =
                    uastc_rdo(tex.get_total_blocks(), (basist::uastc_block*)tex.get_ptr(), (const color_rgba*)m_source_blocks[slice_desc.m_first_block_index].m_pixels, rdo_params, m_params.m_pack_uastc_flags,
                              m_params.m_rdo_uastc_multithreading ? m_params.m_pJob_pool : nullptr, (m_params.m_rdo_uastc_multithreading && m_params.m_pJob_pool) ? basisu::minimum<uint32_t>(4, (uint32_t)m_params.m_pJob_pool->get_total_threads()) : 0);
                if (!status)
                {
    ...
    bool basis_compressor::create_basis_file_and_transcode()
    {
        ...
        if (m_params.m_validate_output_data)
        {
            ...
            for (uint32_t i = 0; i < m_slice_descs.size(); i++)
            {
                ...
                decoded_texture.init(tex_format, m_slice_descs[i].m_width, m_slice_descs[i].m_height);

                tm.start();

                uint32_t bytes_per_block = m_params.m_uastc ? 16 : 8;

                if (!decoder.transcode_slice(&comp_data[0], (uint32_t)comp_data.size(), i, reinterpret_cast<etc_block*>(decoded_texture.get_ptr()), m_slice_descs[i].m_num_blocks_x * m_slice_descs[i].m_num_blocks_y, format, bytes_per_block))
                {
    ...
    static uint8_t g_ktx2_uastc_nonalpha_dfd[44] = {0x2C, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x2,  0x0, 0x28, 0x0, 0xA6, 0x1, 0x2, 0x0, 0x3, 0x3, 0x0,  0x0,  0x10, 0x0,
                                                    0x0,  0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x7F, 0x4, 0x0,  0x0, 0x0,  0x0, 0x0, 0x0, 0x0, 0x0, 0xFF, 0xFF, 0xFF, 0xFF};
    static uint8_t g_ktx2_uastc_alpha_dfd[44]    = {0x2C, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x2,  0x0, 0x28, 0x0, 0xA6, 0x1, 0x2, 0x0, 0x3, 0x3, 0x0,  0x0,  0x10, 0x0,
    ...
    static void* basis_compress(const basisu::vector<image>* pSource_images, const basisu::vector<imagef>* pSource_images_hdr, uint32_t flags_and_quality, float uastc_rdo_quality, size_t* pSize, image_stats* pStats)
    {
    ...
    void* basis_compress(const basisu::vector<image>& source_images, uint32_t flags_and_quality, float uastc_rdo_quality, size_t* pSize, image_stats* pStats)
    {
        return basis_compress(&source_images, nullptr, flags_and_quality, uastc_rdo_quality, pSize, pStats);
    }

    void* basis_compress(const basisu::vector<imagef>& source_images_hdr, uint32_t flags_and_quality, size_t* pSize, image_stats* pStats) { return basis_compress(nullptr, &source_images_hdr, flags_and_quality, 0.0f, pSize, pStats); }

    void* basis_compress(const uint8_t* pImageRGBA, uint32_t width, uint32_t height, uint32_t pitch_in_pixels, uint32_t flags_and_quality, float uastc_rdo_quality, size_t* pSize, image_stats* pStats)
    {
        if (!pitch_in_pixels)
    ...
    bool basis_benchmark_etc1s_opencl(bool* pOpenCL_failed)
    {
        ...
        {
            tm.start();
            void*  pComp_data = basis_compress(images, flags_and_quality, 1.0f, &comp_size, &stats);
        ...
        {
            tm.start();
            void* pComp_data = basis_compress(images, flags_and_quality | cFlagUseOpenCL, 1.0f, &comp_size, &stats);
