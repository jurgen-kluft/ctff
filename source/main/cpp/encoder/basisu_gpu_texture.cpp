...
#include "ctff/private/encoder/basisu_bc7enc.h"
#include "ctff/private/transcoder/basisu_astc_hdr_core.h"

#define TINYDDS_IMPLEMENTATION
#include "ctff/private/3rdparty/tinydds.h"
...
namespace basisu
{
    ...
    struct bc4_block
    {
        ...
        {
            return ((uint64_t)((uint32_t)m_selectors[0] | ((uint32_t)m_selectors[1] << 8U) | ((uint32_t)m_selectors[2] << 16U) | ((uint32_t)m_selectors[3] << 24U))) | (((uint64_t)m_selectors[4]) << 32U) | (((uint64_t)m_selectors[5]) << 40U);
        }
    ...
    {
        assert(w < 8);
        return (l * (64 - basist::g_bc7_weights3[w]) + h * basist::g_bc7_weights3[w] + 32) >> 6;
    }
    static inline uint32_t bc7_interp4(uint32_t l, uint32_t h, uint32_t w)
    {
        assert(w < 16);
        return (l * (64 - basist::g_bc7_weights4[w]) + h * basist::g_bc7_weights4[w] + 32) >> 6;
    }
    ...
    bool unpack_bc7_mode0_2(uint32_t mode, const void* pBlock_bits, color_rgba* pPixels)
    {
        ...
        if (read_bits32(pBuf, bit_offset, mode + 1) != (1U << mode))
            return false;
    ...
    bool unpack_bc7_mode6(const void* pBlock_bits, color_rgba* pPixels)
    {
        ...
        for (uint32_t i = 0; i < 16; i++)
        {
            ...
            vals[i].set_noclamp_rgba((r0 * iw + r1 * w + 32) >> 6, (g0 * iw + g1 * w + 32) >> 6, (b0 * iw + b1 * w + 32) >> 6, (a0 * iw + a1 * w + 32) >> 6);
        }
    ...
    bool unpack_bc7(const void* pBlock, color_rgba* pPixels)
    {
        ...
        for (uint32_t mode = 0; mode <= 7; mode++)
        {
            if (first_byte & (1U << mode))
            {
                ...
                {
                    case 0:
                    case 2: return unpack_bc7_mode0_2(mode, pBlock, pPixels);
                    case 1:
                    case 3:
                    case 7: return unpack_bc7_mode1_3_7(mode, pBlock, pPixels);
                    case 4:
                    case 5: return unpack_bc7_mode4_5(mode, pBlock, pPixels);
                    case 6: return unpack_bc7_mode6(pBlock, pPixels);
                    default: break;
                }
            }
        }
    ...
    static inline int bc6h_interpolate(int a, int b, const uint8_t* pWeights, int index) { return (a * (64 - (int)pWeights[index]) + b * (int)pWeights[index] + 32) >> 6; }
    ...
    static color_rgba expand_565(const color_rgba& c) { return color_rgba((c.r << 3) | (c.r >> 2), (c.g << 2) | (c.g >> 4), (c.b << 3) | (c.b >> 2), 255); }
    ...
    static color_rgba convert_rgb_555_to_888(const color_rgba& col) { return color_rgba((col[0] << 3) | (col[0] >> 2), (col[1] << 3) | (col[1] >> 2), (col[2] << 3) | (col[2] >> 2), 255); }

    static color_rgba convert_rgba_5554_to_8888(const color_rgba& col) { return color_rgba((col[0] << 3) | (col[0] >> 2), (col[1] << 3) | (col[1] >> 2), (col[2] << 3) | (col[2] >> 2), (col[3] << 4) | col[3]); }
    ...
    bool unpack_pvrtc2(const void* p, color_rgba* pPixels)
    {
        ...
        {
            // colora=4433
            color_rgba color_a((pBlock->m_trans_color_data.m_red_a << 1) | (pBlock->m_trans_color_data.m_red_a >> 3), (pBlock->m_trans_color_data.m_green_a << 1) | (pBlock->m_trans_color_data.m_green_a >> 3),
                               (pBlock->m_trans_color_data.m_blue_a << 2) | (pBlock->m_trans_color_data.m_blue_a >> 1), pBlock->m_trans_color_data.m_alpha_a << 1);

            // colorb=4443
            color_rgba color_b((pBlock->m_trans_color_data.m_red_b << 1) | (pBlock->m_trans_color_data.m_red_b >> 3), (pBlock->m_trans_color_data.m_green_b << 1) | (pBlock->m_trans_color_data.m_green_b >> 3),
                               (pBlock->m_trans_color_data.m_blue_b << 1) | (pBlock->m_trans_color_data.m_blue_b >> 3), (pBlock->m_trans_color_data.m_alpha_b << 1) | 1);
    ...
    struct etc2_eac_r11
    {
        ...
        uint64_t get_sels() const { return ((uint64_t)m_sels_0 << 40U) | ((uint64_t)m_sels_1 << 32U) | ((uint64_t)m_sels_2 << 24U) | ((uint64_t)m_sels_3 << 16U) | ((uint64_t)m_sels_4 << 8U) | m_sels_5; }
    ...
    void unpack_uastc(const void* p, color_rgba* pPixels) { basist::unpack_uastc(*static_cast<const basist::uastc_block*>(p), (basist::color32*)pPixels, false); }
    ...
    {
        if ((m_fmt != texture_format::cASTC_HDR_4x4) && (m_fmt != texture_format::cUASTC_HDR_4x4) && (m_fmt != texture_format::cASTC_HDR_6x6) && (m_fmt != texture_format::cBC6HUnsigned) && (m_fmt != texture_format::cBC6HSigned))
        {
    ...
    bool create_ktx_texture_file(uint8_vec& ktx_data, const basisu::vector<gpu_image_vec>& gpu_images, bool cubemap_flag)
    {
        ...
        for (uint32_t array_index = 0; array_index < gpu_images.size(); array_index++)
        {
            ...
            {
                if ((width != levels[0].get_pixel_width()) || (height != levels[0].get_pixel_height()) || (total_levels != levels.size()))
                {
            ...
            for (uint32_t level_index = 0; level_index < levels.size(); level_index++)
            {
                if (level_index)
                {
                    if ((levels[level_index].get_pixel_width() != maximum<uint32_t>(1, levels[0].get_pixel_width() >> level_index)) || (levels[level_index].get_pixel_height() != maximum<uint32_t>(1, levels[0].get_pixel_height() >> level_index)))
                    {
    ...
    bool does_dds_support_format(texture_format fmt)
    {
        switch (fmt)
        {
            ...
            case texture_format::cBC6HSigned:
            case texture_format::cBC6HUnsigned:
            case texture_format::cBC7: return true;
            default: break;
        }
        return false;
    }
    ...
    bool write_dds_file(uint8_vec& dds_data, const basisu::vector<gpu_image_vec>& gpu_images, bool cubemap_flag, bool use_srgb_format)
    {
        ...
        cbs.error = [](void* user, char const* msg)
        {
            BASISU_NOTE_UNUSED(user);
            fprintf(stderr, "tinydds: %s\n", msg);
        };
        cbs.alloc = [](void* user, size_t size) -> void*
        {
            BASISU_NOTE_UNUSED(user);
            return malloc(size);
        };
        cbs.free = [](void* user, void* memory)
        {
            BASISU_NOTE_UNUSED(user);
            free(memory);
        };
        cbs.write = [](void* user, void const* buffer, size_t byteCount)
        {
            BASISU_NOTE_UNUSED(user);
            uint8_vec* pVec = (uint8_vec*)user;
            append_vector(*pVec, (const uint8_t*)buffer, byteCount);
        };
        ...
        switch (fmt)
        {
            case texture_format::cBC1_NV:
            case texture_format::cBC1_AMD:
            case texture_format::cBC1: tinydds_fmt = use_srgb_format ? TDDS_BC1_RGBA_SRGB_BLOCK : TDDS_BC1_RGBA_UNORM_BLOCK; break;
            case texture_format::cBC3: tinydds_fmt = use_srgb_format ? TDDS_BC3_SRGB_BLOCK : TDDS_BC3_UNORM_BLOCK; break;
            case texture_format::cBC4: tinydds_fmt = TDDS_BC4_UNORM_BLOCK; break;
            case texture_format::cBC5: tinydds_fmt = TDDS_BC5_UNORM_BLOCK; break;
            case texture_format::cBC6HSigned: tinydds_fmt = TDDS_BC6H_SFLOAT_BLOCK; break;
            case texture_format::cBC6HUnsigned: tinydds_fmt = TDDS_BC6H_UFLOAT_BLOCK; break;
            case texture_format::cBC7: tinydds_fmt = use_srgb_format ? TDDS_BC7_SRGB_BLOCK : TDDS_BC7_UNORM_BLOCK; break;
            default:
            {
        ...
        bool status = TinyDDS_WriteImage(&cbs, &dds_data, width, height, 1, slices, total_levels, (TinyDDS_Format)tinydds_fmt, cubemap_flag, true, mipmap_sizes, mipmap_ptrs);
    ...
    bool read_uncompressed_dds_file(const char* pFilename, basisu::vector<image>& ldr_mips, basisu::vector<imagef>& hdr_mips)
    {
        ...
        cbs.errorFn = [](void* user, char const* msg)
        {
            BASISU_NOTE_UNUSED(user);
            fprintf(stderr, "tinydds: %s\n", msg);
        };
        cbs.allocFn = [](void* user, size_t size) -> void*
        {
            BASISU_NOTE_UNUSED(user);
            return malloc(size);
        };
        cbs.freeFn = [](void* user, void* memory)
        {
            BASISU_NOTE_UNUSED(user);
            free(memory);
        };
        ...
        switch (tfmt)
        {
            ...
            case TDDS_B8G8R8A8_UNORM: fmt = cRGBA32; break;
            ...
            case TDDS_R32G32B32A32_SFLOAT:
                ...
                hdr_flag = true;
                break;
            default: error_printf("File \"%s\" has an unsupported DDS texture format (only supports RGBA/BGRA 32bpp, RGBA HALF float, or RGBA FLOAT)\n", pFilename); goto failure;
        }
